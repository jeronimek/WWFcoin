...
// Copyright (c) 2011-2012 The WWFcoin Developers
...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "WWFcoin server has now stopped running!";
}
...
// WWFcoin: Return average network hashes per second based on last number of blocks.
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <WWFcoin address> <account>\n"
            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid WWFcoin address");
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress <WWFcoin address> <amount> [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <WWFcoin address> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <WWFcoin address> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <WWFcoin address> [minconf=1]\n"
            "Returns the total amount received by <WWFcoin address> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <to WWFcoin address> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid WWFcoin address:")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a WWFcoin address or hex-encoded public key\n"
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        // Case 1: WWFcoin address and we have full public key:
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; WWFcoin server stopping, restart to run with encrypted wallet";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <WWFcoin address>\n"
            "Return information about <WWFcoin address>.");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "WWFcoin server is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "WWFcoin server is downloading blocks...");
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: WWFcoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
//
// JSON-RPC protocol.  WWFcoin speaks version 1.0 for maximum compatibility,
...
void ThreadRPCServer2(void* parg)
{
    ...
    if (mapArgs["-rpcpassword"] == "")
    {
        ...
        string strWhatAmI = "To use WWFcoin";
    ...
    ip::tcp::endpoint endpoint(bindAddress, GetArg("-rpcport", 8120));
...
Object CallRPC(const string& strMethod, const Array& params)
{
    ...
    if (!d.connect(GetArg("-rpcconnect", "127.0.0.1"), GetArg("-rpcport", "8120")))
